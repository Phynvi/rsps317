package com.bclaus.rsps.server.vd.items.container;
...
public class ItemContainer implements Iterable<Item> {
    ...
    public boolean add(Item item, int slot) {
        if (!Item.valid(item) || item.getCount() > Integer.MAX_VALUE) {
        ...
        if (item.getCount() > remainingSlots && !item.getDefinition().isStackable()) {
    ...
    public boolean contains(Item item) {
        return stream().filter(Objects::nonNull).anyMatch(i -> i.getId() == item.getId() && i.getCount() >= item.getCount());
    }
    ...
    public int amount(int id) {
        return stream().filter(i -> Item.valid(i) && i.getId() == id).mapToInt(i -> i.getCount()).sum();
    }
