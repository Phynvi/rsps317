...
public class Smithing {
	...
	public static void startSmelting(Player c, int barType) {
		if (canSmelt(c, barType)) {
			// c.sendMessage("We canSmelt");
			if (hasOres(c, barType)) {
	...
	public static void smelt(Player c, int barType) {
		if (c.smeltAmount > 0) {
			c.getPA().closeAllWindows();
			if (hasOres(c, barType)) {
				...
				c.getPA().addSkillXP(exp * Constants.SMITHING_EXPERIENCE, Player.playerSmithing);
	...
	public static boolean canSmelt(Player c, int barType) {
		for (int j = 0; j < SMELT_BARS.length; j++) {
			if (barType == SMELT_BARS[j]) {
	...
	public static boolean hasOres(Player c, int barType) {
	...
	public static void readInput(Player c, int level, String type, int amounttomake) {
	...
	private static void CheckIron(Player c, int level, int amounttomake, String type) {
		...
		else if (type.equals("2") && level >= 35)
		{
			xp = 50;
			item = 2;
			removeamount = 1;
	...
	private static void CheckSteel(Player c, int level, int amounttomake, String type) {
		...
		} else if (type.equals("1119") && level >= 48) // Platebody
		{
			...
			removeamount = 5;
			maketimes = amounttomake;
		} else if (type.equals("1096") && level >= 45) {
			xp = 175;
			item = 2;
			removeamount = 1;
			maketimes = amounttomake;
	...
	private static void CheckMith(Player c, int level, int amounttomake, String type) {
		remove = 2359;
	...
	public static boolean doaction(Player c, int toadd, int toremove, int toremove2, int timestomake, int NOTUSED, int NOTUSED2, int xp) {
