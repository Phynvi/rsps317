...
public class CastOnOther extends MagicRequirements {
	...
	public static void healOther(Player c, int i) {
		...
		c.damage(new Hit((int) hpPercent));
	...
	public static void castOtherVengeance(Player c, int i) {
		...
		if (p.inWild()) {
			c.sendMessage("You cannot cast this spell here..");
			return;
		}
		if (p.vengOn) {
		...
		/*
	 * public static void teleOtherDistance(Client c, int type, int i) { Client
	 * castOn = (Client) PlayerHandler.PLAYERS.get(i]; int[][] data = { { 74,
	 * SOUL, LAW, EARTH, 1, 1, 1 }, { 82, SOUL, LAW, WATER, 1, 1, 1 }, { 90,
	 * SOUL, LAW, -1, 2, 1, -1 }, }; if (!hasRequiredLevel(c, data[type][0])) {
	 * c.sendMessage("You need to have a magic level of " + data[type][0] +
	 * " to cast this spell."); return; } if (!hasRunes(c, new int[] {
	 * data[type][1], data[type][2], data[type][3] }, new int[] { data[type][4],
	 * data[type][5], data[type][6] })) { return; } deleteRunes(c, new int[] {
	 * data[type][1], data[type][2], data[type][3] }, new int[] { data[type][4],
	 * data[type][5], data[type][6] }); String[] location = { "Lumbridge",
	 * "Falador", "Camelot", }; c.faceUpdate(i + 32768); c.startAnimation(1818);
	 * c.gfx0(343); if (castOn != null) { if (castOn.distanceToPoint(c.absX,
	 * c.absY) <= 15) { if (c.heightLevel == castOn.heightLevel) {
	 * castOn.getPA().sendFrame126(location[type], 12560);
	 * castOn.getPA().sendFrame126(c.playerName, 12558);
	 * castOn.getPA().showInterface(12468); castOn.teleotherType = type; } } } }
	 * 
	 * /* public static void teleOtherLocation(final Client c, final int i,
	 * boolean decline) { c.getPA().removeAllWindows(); final int[][] coords = {
	 * {3222, 3218}, // LUMBRIDGE {2964, 3378}, // FALADOR {2757, 3477}, //
	 * CAMELOT }; if(!decline) { CycleEventHandler.getSingleton().addEvent(c,
	 * new CycleEvent() { public void execute(CycleEventContainer container) {
	 * c.startAnimation(715); c.teleportToX = coords[c.teleotherType][0];
	 * c.teleportToY = coords[c.teleotherType][1]; c.teleotherType = -1;
	 * container.stop(); } public void stop() {
	 * 
	 * } }, 3); c.startAnimation(1816); c.gfx100(342); }
	 */
